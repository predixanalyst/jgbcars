...
/* ============================================================
   VEHÍCULOS
   ============================================================ */
function Vehiculos({ vehicles, vehiclesLoading, addVehicle, deleteVehicle }) {
  const [q, setQ] = useState("");
  const [estadoF, setEstadoF] = useState("Todos");
  ...
  const saveNew = async () => {
    ...
    const nuevo = {
      ...
      proveedor: form.proveedor || "—", cliente: "—", fCompra: new Date().toISOString().slice(0, 10), fVenta: null,
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected?.mode === "new" ? "Añadir vehículo" : `${selected?.v?.marca} ${selected?.v?.modelo}`} wide>
        {selected?.mode === "new" ? (
          <div>
            ...
            <PrimaryBtn full onClick={saveNew}>{saving ? "Guardando…" : "Guardar vehículo"}</PrimaryBtn>
          </div>
        ) : selected?.v && (
          <div>
...
/* ============================================================
   PRIVATE SHELL
   ============================================================ */
function PrivateArea({ worker, onLogout, vehicles, vehiclesLoading, addVehicle, deleteVehicle }) {
  const [page, setPage] = useState("dashboard");
  const [mobileOpen, setMobileOpen] = useState(false);
  const pages = {
    dashboard: <DashboardHome worker={worker} />,
    contabilidad: <Contabilidad />,
    vehiculos: <Vehiculos vehicles={vehicles} vehiclesLoading={vehiclesLoading} addVehicle={addVehicle} deleteVehicle={deleteVehicle} />,
    clientes: <Clientes />,
    tareas: <Tareas />,
  ...
  React.useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        // Si hay error o la tabla está vacía, se quedan los datos de demostración
        // como respaldo, para que la web nunca se quede sin nada que mostrar.
