...
function VehiclePhoto({ v, tone, index = 0 }) {
  ...
  const fotos = v.fotos && v.fotos.length > 0 ? v.fotos : (v.foto ? [v.foto] : []);
  const src = fotos[index] || fotos[0] || `https://loremflickr.com/640/480/${slug}/all?lock=${lock}`;
  return (
    <img
      src={src}
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        <div>
          <div className="relative rounded-xl overflow-hidden mb-3" style={{ aspectRatio: "4 / 3" }}>
            {fotos.length > 0 ? (
              <img src={fotos[activeFoto]} alt={`${v.marca} ${v.modelo}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
            ) : (
          ...
          {fotos.length > 1 && (
            <div className="grid grid-cols-4 gap-2">
              {fotos.map((f, i) => (
                <button key={f} onClick={() => setActiveFoto(i)} className="rounded-lg overflow-hidden" style={{ aspectRatio: "4 / 3", border: i === activeFoto ? `2px solid ${C.accent}` : `1px solid ${C.border}` }}>
                  <img src={f} alt={`${v.marca} ${v.modelo} ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                </button>
              ))}
            </div>
          )}
...
function Vehiculos({ vehicles, vehiclesLoading, addVehicle, deleteVehicle, updateVehicle }) {
  ...
  const openEdit = (v) => {
    setForm({
      ...
      fotos: v.fotos && v.fotos.length > 0 ? v.fotos : (v.foto ? [v.foto] : []),
    });
    setPhotoError("");
  ...
  const handlePhotoUpload = async (e) => {
    ...
    setForm((f) => ({ ...f, fotos: [...f.fotos, ...nuevasUrls] }));
    setUploadingPhoto(false);
    e.target.value = "";
  };

  const removePhoto = (url) => {
    setForm((f) => ({ ...f, fotos: f.fotos.filter((x) => x !== url) }));
  };
  ...
  const saveNew = async () => {
    ...
    const nuevo = {
      ...
      cambio: "Automático", tag: null, fotos: form.fotos, foto: form.fotos[0] || null,
  ...
  const saveEdit = async () => {
    ...
    const actualizado = {
      ...
      estado: form.estado, proveedor: form.proveedor || "—", fotos: form.fotos, foto: form.fotos[0] || null,
    };
    setSaving(true);
                ...
                <Field label="Fotos del vehículo (puedes elegir varias a la vez)">
                  ...
                  {form.fotos.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {form.fotos.map((url) => (
                        <div key={url} className="relative rounded-lg overflow-hidden" style={{ width: 90, aspectRatio: "4 / 3" }}>
