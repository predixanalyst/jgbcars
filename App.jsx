          ...
          <div className="grid grid-cols-4 gap-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="rounded-lg overflow-hidden" style={{ aspectRatio: "4 / 3" }}>
                <img src={`https://loremflickr.com/320/240/${MARCA_SLUG[v.marca] || v.marca.toLowerCase()}/all?lock=${Number(lock) + i * 37}`}
                  alt={`${v.marca} ${v.modelo} ${i + 1}`} loading="lazy"
                  style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}
      ...
      <div className="mb-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {EQUIPMENT.map(({ cat, items }) => (
            <div key={cat} className="rounded-xl p-4" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              <div className="font-bold text-sm mb-2.5" style={{ color: C.accent }}>{cat}</div>
              {items.map((it) => (
                <div key={it} className="text-xs mb-1.5" style={{ color: C.muted }}>✔ {it}</div>
              ))}
            </div>
          ))}
        </div>
...
function Contabilidad() {
  ...
  const toggleEstado = (id) => setInvoices(invoices.map((f) => f.id === id ? { ...f, estado: f.estado === "Pagada" ? "Pendiente" : "Pagada" } : f));
  const remove = (id) => setInvoices(invoices.filter((f) => f.id !== id));
  ...
  return (
    <div className="jgb-fade">
      ...
      <p className="text-sm mb-6" style={{ color: C.muted }}>Gestión económica interna de la empresa.</p>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard label="Balance mensual (ago)" value={fmtEUR(MONTHLY[5].ingresos - MONTHLY[5].gastos)} accentColor={C.accent} />
      </div>
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
        <div className="font-bold text-sm">Facturas ({filtered.length})</div>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "new" ? "Nueva factura" : `Factura ${modal?.f?.id}`}>
        {modal?.mode === "new" ? (
          <div>
            <Field label="Cliente"><input style={inputStyle} placeholder="Nombre del cliente" /></Field>
            <Field label="Concepto"><input style={inputStyle} placeholder="Descripción del servicio o venta" /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Base imponible"><input style={inputStyle} placeholder="0,00 €" /></Field>
              <Field label="IVA %"><input style={inputStyle} placeholder="21" /></Field>
            </div>
            <PrimaryBtn full onClick={() => setModal(null)}>Crear factura (demo)</PrimaryBtn>
          </div>
        ) : (
...
/* ============================================================
   VEHÍCULOS
   ============================================================ */
function Vehiculos({ vehicles, setVehicles }) {
  const [q, setQ] = useState("");
  const [estadoF, setEstadoF] = useState("Todos");
  ...
  const blankForm = {
    ...
    compra: 0, gastos: 0, venta: 0, proveedor: "", estado: "Disponible", foto: "",
  };
  ...
  const openNew = () => { setForm(blankForm); setSelected({ mode: "new" }); };

  const saveNew = () => {
    if (!form.marca.trim() || !form.modelo.trim()) return;
    const nextNum = vehicles.length + 1;
    const nuevo = {
      id: `V-${String(nextNum).padStart(3, "0")}-${Date.now().toString().slice(-4)}`,
      ...
      cambio: "Automático", tag: null, foto: form.foto.trim() || null,
      combustible: "Diésel", potencia: "—", traccion: "Delantera", colorExterior: "—", colorInterior: "—",
    };
    setVehicles([nuevo, ...vehicles]);
    setSelected(null);
  };
  ...
  return (
    <div className="jgb-fade">
      ...
      <p className="text-sm mb-5" style={{ color: C.muted }}>{filtered.length} vehículos en el sistema. Los que están "Disponible", "Reservado" o "Vendido" se ven también en la web pública, en Stock.</p>
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected?.mode === "new" ? "Añadir vehículo" : `${selected?.v?.marca} ${selected?.v?.modelo}`} wide>
        {selected?.mode === "new" ? (
          <div>
            <div className="grid grid-cols-2 gap-x-4">
              ...
              <div className="col-span-2">
                <Field label="Foto (pega aquí el enlace a la imagen, como en Wallapop)">
                  <input style={inputStyle} placeholder="https://…" value={form.foto} onChange={(e) => setForm({ ...form, foto: e.target.value })} />
                </Field>
              </div>
            </div>
            ...
            <PrimaryBtn full onClick={saveNew}>Guardar vehículo</PrimaryBtn>
          </div>
        ) : selected?.v && (
          <div>
            ...
            <div className="flex gap-2 mt-2">
              <GhostBtn full><Pencil size={14} className="inline -mt-0.5 mr-1" />Editar</GhostBtn>
              <GhostBtn full onClick={() => { setVehicles(vehicles.filter((x) => x.id !== selected.v.id)); setSelected(null); }}>
  ...
  return (
    <div className="jgb-fade">
      <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
        ...
        <PrimaryBtn><Plus size={14} className="inline -mt-0.5 mr-1" />Nuevo cliente</PrimaryBtn>
      </div>
      <p className="text-sm mb-5" style={{ color: C.muted }}>CRM interno · {filtered.length} clientes.</p>
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="jgb-row" style={{ borderTop: `1px solid ${C.border}` }}>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => setSelected(c)} style={{ color: C.accent }} className="text-xs font-semibold">Ver ficha</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected ? `${selected.nombre} ${selected.apellidos}` : ""}>
        {selected && (
          <div className="text-sm space-y-2">
            {[["Empresa", selected.empresa], ["NIF/CIF", selected.nif], ["Teléfono", selected.telefono], ["Email", selected.email], ["Dirección", selected.direccion]].map(([l, v]) => (
            ...
            <div className="pt-2">
              ...
              {selected.vehiculos.length ? selected.vehiculos.map((id) => <Badge key={id} tone="accent">{id}</Badge>) : <span style={{ color: C.mutedDim }}>Ninguno</span>}
            </div>
            <div className="pt-2 text-xs" style={{ color: C.muted }}><span style={{ color: C.mutedDim }}>Notas: </span>{selected.notas}</div>
          </div>
        )}
      </Modal>
    </div>
  ...
  return (
    <div className="jgb-fade">
      <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
        ...
        <PrimaryBtn><Plus size={14} className="inline -mt-0.5 mr-1" />Nueva tarea</PrimaryBtn>
      </div>
      <p className="text-sm mb-5" style={{ color: C.muted }}>Vista Kanban del equipo.</p>
              ...
              <tr key={d.id} className="jgb-row" style={{ borderTop: `1px solid ${C.border}` }}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setDocs(docs.filter((x) => x.id !== d.id))} style={{ color: C.danger }} title="Eliminar"><Trash2 size={15} /></button>
                  </div>
                </td>
              </tr>
            ))}
  ...
  return (
    <div className="jgb-fade">
      <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
        ...
        <PrimaryBtn><Plus size={14} className="inline -mt-0.5 mr-1" />Nuevo evento</PrimaryBtn>
      </div>
      <p className="text-sm mb-5" style={{ color: C.muted }}>Citas, entregas, revisiones y recordatorios.</p>
...
function Comunicaciones({ worker }) {
  const [msgs, setMsgs] = useState(MESSAGES_SEED);
  const [txt, setTxt] = useState("");
  const send = () => {
    if (!txt.trim()) return;
    setMsgs([{ id: `M-${msgs.length + 1}`, de: worker.name, texto: txt, fecha: "ahora", tipo: "Equipo" }, ...msgs]);
    setTxt("");
  };
  return (
    <div className="jgb-fade">
      <h2 className="text-2xl font-bold mb-1">Comunicaciones</h2>
      <p className="text-sm mb-5" style={{ color: C.muted }}>Mensajes internos y avisos generales.</p>
...
/* ============================================================
   PRIVATE SHELL
   ============================================================ */
function PrivateArea({ worker, onLogout, vehicles, setVehicles }) {
  const [page, setPage] = useState("dashboard");
  const [mobileOpen, setMobileOpen] = useState(false);
  const pages = {
    dashboard: <DashboardHome worker={worker} />,
    contabilidad: <Contabilidad />,
    vehiculos: <Vehiculos vehicles={vehicles} setVehicles={setVehicles} />,
    clientes: <Clientes />,
    tareas: <Tareas />,
...
export default function JGBCarsApp() {
  ...
  return (
    <>
      ...
      {mode === "private" && worker && <PrivateArea worker={worker} onLogout={() => { setWorker(null); setMode("public"); }} vehicles={vehicles} setVehicles={setVehicles} />}
    </>
  );
}
