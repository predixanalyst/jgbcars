...
export function dbRowToVehicle(row) {
  const fotosRaw = row.fotos || (row.foto ? [row.foto] : []);
  const fotos = fotosRaw.map((f) => (typeof f === "string" ? { url: f, position: "50% 50%" } : f));
  return {
    id: row.id,
    marca: row.marca,
    modelo: row.modelo,
...
export function vehicleToDbRow(v) {
  const fotos = (v.fotos || []).map((f) => (typeof f === "string" ? { url: f, position: "50% 50%" } : f));
  return {
    id: v.id,
    marca: v.marca,
    modelo: v.modelo,
    ...
    cambio: v.cambio,
    tag: v.tag,
    foto: (fotos[0] && fotos[0].url) || v.foto || null,
    fotos,
    equipamiento: v.equipamiento || [],
    orden: v.orden ?? 999999,
