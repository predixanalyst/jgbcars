...
function Vehiculos({ vehicles, vehiclesLoading, addVehicle, deleteVehicle }) {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    if (error) {
      setPhotoError("No se ha podido subir la foto. Inténtalo de nuevo.");
      setUploadingPhoto(false);
      return;
    }
    const { data: urlData } = supabase.storage.from("vehiculos").getPublicUrl(fileName);
    setForm((f) => ({ ...f, foto: urlData.publicUrl }));
    setUploadingPhoto(false);
  };
          ...
          <div>
            <div className="grid grid-cols-2 gap-x-4">
              ...
              <div className="col-span-2">
                <Field label="Foto del vehículo">
                  <input type="file" accept="image/*" style={inputStyle} onChange={handlePhotoUpload} disabled={uploadingPhoto} />
                  {uploadingPhoto && <div className="text-xs mt-1" style={{ color: C.mutedDim }}>Subiendo foto…</div>}
                  {photoError && <div className="text-xs mt-1" style={{ color: C.danger }}>{photoError}</div>}
                  {form.foto && !uploadingPhoto && (
                    <div className="mt-2 rounded-lg overflow-hidden" style={{ width: 120, aspectRatio: "4 / 3" }}>
                      <img src={form.foto} alt="Vista previa" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    </div>
                  )}
                </Field>
              </div>
            ...
            <PrimaryBtn full onClick={() => { if (!uploadingPhoto) saveNew(); }}>{saving ? "Guardando…" : uploadingPhoto ? "Esperando la foto…" : "Guardar vehículo"}</PrimaryBtn>
          </div>
        ) : selected?.v && (
          <div>
